...
class FoodSize(BaseMenuModel):
    ...
    name: Mapped[BaseModelFieldTypes.str_255]
    parent_id: Mapped[int] = mapped_column(ForeignKey("menu.food.id"))
    is_new: Mapped[bool] = mapped_column(default=False)
    price: Mapped[float]

    parent_food: Mapped["Food"] = relationship(back_populates="sizes")
...
class ModifierOption(BaseMenuModel):
    ...
    name: Mapped[BaseModelFieldTypes.str_255]
    food_modifier_option_id: Mapped[int] = mapped_column(ForeignKey("menu.modifier_category.id"))
    price: Mapped[float]
